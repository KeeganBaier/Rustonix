...
{
    [Info("HeliControl", "Shady", "1.2.6", ResourceId = 1348)]
    ...
    class HeliControl : RustPlugin
    {
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            if (Config.Any(p => p.Key == "Spawning - Automatically call helicopter after this many seconds"))
            {
                var oldTime = GetConfig("Spawning - Automatically call helicopter after this many seconds", 0f);
                Config["Spawning - Automatically call helicopter between min seconds"] = oldTime;
                Config["Spawning - Automatically call helicopter between max seconds"] = oldTime;
                MinSpawnTime = oldTime;
                MaxSpawnTime = oldTime;
                Config.Remove("Spawning - Automatically call helicopter after this many seconds");
            }
            SaveConfig();
        }
        void Init()
        ...
        void OnServerInitialized()
        {
            ...
            foreach (var ent in BaseEntity.saveList)
            {
                ...
                var prefabName = ent.ShortPrefabName;
            ...
            if (rngTime > 0f)
            {
                if (!UseOldSpawning)
                {
                    timer.Once(rngTime, () =>
                    {
                        if (HeliCount > 0 && !AutoCallIfExists) return;
                        TimerHeli = callHeli();
                    });
                }
                else timer.Every(rngTime, () =>
                {
                    if (HeliCount > 0 && !AutoCallIfExists) return;
                    callHeli();
                });
        ...
        private void cmdTeleportHeli(IPlayer player, string command, string[] args)
        {
            ...
            TeleportPlayer(ply, GetGround(HeliInstance.transform.position));
        ...
        private float GetRandomSpawnTime() { return (MinSpawnTime > 0 && MaxSpawnTime > 0 && MaxSpawnTime >= MinSpawnTime) ? UnityEngine.Random.Range(MinSpawnTime, MaxSpawnTime) : -1f; }
